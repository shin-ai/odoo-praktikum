...
class TrainingCourse(models.Model):
    ...
    user_id = fields.Many2one('res.users', string="Penaggung Jawab", default=lambda self: self.env.uid, tracking=True)
    session_line = fields.One2many('training.session', 'course_id', string='Sesi', tracking=True)
    product_ids = fields.Many2many('product.product', 'course_product_rel', 'course_id', 'procuct_id', string='Cendera Mata', tracking=True)
    level = fields.Selection([
        ('basic', 'Dasar'), 
        ('advanced', 'Lanjutan')
    ], string='Tingkatan', default='basic')
...
class TrainingSession(models.Model):
    ...
    course_id = fields.Many2one('training.course', string="Judul Kursus", reqired=True, ondelete="cascade")
    name = fields.Char(string='Nama', required=True)
    start_date = fields.Date(string='Tanggal')
    duration = fields.Float(string='Durasi', help='Jumlah Hari Training', default=1)
    seats = fields.Integer(string='Kursi', help='Jumlah Kouta Kursi')
...
class TrainingAttendee(models.Model):
    ...
    gender = fields.Selection([
        ('male', 'Pria'), 
        ('female', 'Wanita')
    ], string='Gender', required=True, help='Jenis Kelamin')
